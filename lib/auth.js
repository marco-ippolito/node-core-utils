...
async function auth(
  ...
  if (options.mitre) {
    const { mitre_org, mitre_user, mitre_key } = getMergedConfig();
    if (!mitre_org || !mitre_user || !mitre_key) {
      errorExit(
        'Get your MITRE API configuration' +
        'and run the following command to add it to your ncu config: ' +
        'ncu-config --global set mitre_org ORG and ' +
        'ncu-config --global set mitre_user USERNAME ' +
        'ncu-config --global set mitre_key KEY'
      );
    }
    result.mitre = encode(mitre_org, mitre_user, mitre_key);
  }
  return result;
}
