...
export default class Request {
  ...
  mitreReserveCVEid(amount) {
    const url = new URL('https://cveawg-test.mitre.org/api/cve-id');
    url.searchParams.append('batch_type', 'sequential');
    url.searchParams.append('number', 'amount');
    url.searchParams.append('short_name', 'nodejs');  // not sure if this is correct
    url.searchParams.append('cve_year', new Date().getFullYear() + 1);

    const options = {
      method: 'POST',
      headers: {
        'User-Agent': 'node-core-utils',
        'CVE-API-ORG': this.credentials.mitre_org,
        'CVE-API-USER': this.credentials.mitre_user,
        'CVE-API-KEY': this.credentials.mitre_key,
        Accept: 'application/json',
        'Content-Type': 'application/json'
      }
    };
    return this.json(url.href, options);
  }

  getHackerOneReport(id) {
    const url = `https://api.hackerone.com/v1/reports/${id}`;
    const options = {
      method: 'GET',
      headers: {
        Authorization: `Basic ${this.credentials.h1}`,
