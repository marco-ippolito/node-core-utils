...
export default class Request {
  ...
  mitreReserveCVEid() {
    const url = 'https://cveawg-test.mitre.org/api/cve-id'; // test url
    const options = {
      method: 'POST',
      headers: {
        'User-Agent': 'node-core-utils',
      ...
      body: JSON.stringify({
        number: 1,
        short_name: 'nodejs', // not sure if this is correct
        cve_year: new Date().getFullYear() // issue if we are in the last day of the year
      })
    };
    return this.json(url, options);
  }
