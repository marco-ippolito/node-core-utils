...
export default class SecurityReleaseRequestCVEs {
  ...
  async start() {
    ...
    await reserveCVEIds(reports, { cli, credentials });
  }
}

async function reserveCVEIds(reports, { cli, credentials }) {
  const checkedReports = [];

  // manually prompt confirmation for reports that already have CVEs
  for (const report of reports) {
    const checkedReport = await checkIfAlreadyHasCveIds(report, { cli });
    checkedReports.push(checkedReport);
  }
  const cveIdsAmount = checkedReports.filter(report => !report.skipped).length;

  if (!cveIdsAmount) {
    cli.error('No reports to reserve CVEs for');
    process.exit(1);
  }

  const req = new Request(credentials);
  const response = await req.mitreReserveCVEid(cveIdsAmount);
  const { cve_ids } = response;

  if (!cve_ids?.length) {
    // mitre returns an error json in the response body
    throw new Error(JSON.stringify(response));
  }

  // asssign CVEs Ids to reports
  for (const report of checkedReports) {
    if (report.skipped) continue;
    report.cveId = cve_ids.shift();
  }

  return checkedReports;
}

async function checkIfAlreadyHasCveIds(report, { cli }) {
